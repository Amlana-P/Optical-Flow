...
def UpSample(I):
    ...
    Iy[::2, :] = Ix
    Ig = cv2.GaussianBlur(Iy, (5, 5), 1.5,
                          1.5)  # instead of using the user-defined gaussian function, I am using the Gaussian Blur functtion for double the size of gaussian kernel size
    return Ig
...
def LucasKanade(I1, I2):
    ...
    Ix = signal.convolve2d(I1, [[-0.25, 0.25], [-0.25, 0.25]], 'same') + signal.convolve2d(I2, [[-0.25, 0.25],
                                                                                                [-0.25, 0.25]], 'same')
    Iy = signal.convolve2d(I1, [[-0.25, -0.25], [0.25, 0.25]], 'same') + signal.convolve2d(I2, [[-0.25, -0.25],
                                                                                                [0.25, 0.25]], 'same')
    It = signal.convolve2d(I1, [[0.25, 0.25], [0.25, 0.25]], 'same') + signal.convolve2d(I2, [[-0.25, -0.25],
                                                                                              [-0.25, -0.25]], 'same')
...
def LucasKanadeIterative(I1, I2, u1, v1):
    ...
    for i in range(2, S[0] - 2):
        for j in range(2, S[1] - 2):
            ...
            Ix = signal.convolve2d(I1new, [[-0.25, 0.25], [-0.25, 0.25]], 'same') + signal.convolve2d(I2new,
                                                                                                      [[-0.25, 0.25],
                                                                                                       [-0.25, 0.25]],
                                                                                                      'same')
            Iy = signal.convolve2d(I1new, [[-0.25, -0.25], [0.25, 0.25]], 'same') + signal.convolve2d(I2new,
                                                                                                      [[-0.25, -0.25],
                                                                                                       [0.25, 0.25]],
                                                                                                      'same')
            It = signal.convolve2d(I1new, [[0.25, 0.25], [0.25, 0.25]], 'same') + signal.convolve2d(I2new,
                                                                                                    [[-0.25, -0.25],
                                                                                                     [-0.25, -0.25]],
                                                                                                    'same')
...
def LK_Pyramid(Im1, Im2, iteration, level):
    ...
    u1 = UpSample(u)
    v1 = UpSample(v)
    I1new = pyramid1[0:(len(pyramid1[:, 0]) / (2 ** (level - k - 1))), 0:(len(pyramid1[0, :]) / (2 ** (level - k - 1))),
            level - k - 1]
    I2new = pyramid2[0:(len(pyramid2[:, 0]) / (2 ** (level - k - 1))), 0:(len(pyramid2[0, :]) / (2 ** (level - k - 1))),
            level - k - 1]
